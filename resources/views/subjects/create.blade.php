...
@section('page_header', 'Create Subject')
                ...
                <div class="card z-index-0 fadeIn3 fadeInBottom">
                    <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                        <div class="bg-gradient-primary shadow-primary border-radius-lg py-3 pe-1">
                            <h4 class="text-white font-weight-bolder text-center mt-2 mb-0">Create Subject</h4>
                        </div>
                    </div>
                    <div class="card-body">
                                ...
                                <select class="form-control" id="curricula" name="curricula[]" multiple required>
                                    @foreach ($curriculums as $curriculum)
                                        <option value="{{ $curriculum->id }}"
                                            {{ in_array($curriculum->id, old('curricula', [])) ? 'selected' : '' }}>
                                            {{ $curriculum->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-control" id="grade_levels" name="grade_levels[]" multiple required>
                                    @foreach ($gradeLevels as $level)
                                        <option value="{{ $level->id }}"
                                            {{ in_array($level->id, old('grade_levels', [])) ? 'selected' : '' }}>
                                            {{ $level->name }}
                                        </option>
                                    @endforeach
                                </select>
