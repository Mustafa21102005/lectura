...
class MessagesController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        if ($currentUser->hasRole('parent')) {
            // Parents see teachers only
            $query->role('teacher');
        } elseif ($currentUser->hasRole('teacher')) {
            // Teachers see teachers and parents
            $query->role(['teacher', 'parent']);
        }

        $records = $query->paginate($request->per_page ?? $this->perPage);

        // Build HTML output for Chatify
        $getRecords = '';
        ...
        return Response::json([
            'records' => $getRecords ?: '<p class="message-hint center-el"><span>Nothing to show.</span></p>',
            'total' => $records->total(),
